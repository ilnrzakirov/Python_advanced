...
with sqlite3.connect("hw_3_database.db") as conn:
    ...
    cursor.execute("SELECT COUNT(table_1.id) FROM 'table_1' JOIN table_2 t on table_1.id = t.id")
    repetition_table_1_2 = cursor.fetchall()
    cursor.execute("SELECT COUNT(*) FROM 'table_1' JOIN table_2 t on table_1.id = t.id JOIN table_3 t2 on table_1.id = t2.id")
    repetition_table_1_2_3 = cursor.fetchall()
    ...
    print(f"Повторений между 1 и 2 таблицей: {repetition_table_1_2}")
    print(f"Повторений между 1 и 2 и 3 таблицей: {repetition_table_1_2_3}")
